...
from app.config import AppConfig, WEB_SYSTEM_PROMPT
from app.services.location_service import LocationService
from app.services.ollama_client import OllamaClient
from app.utils import contains_trigger_phrase, fetch_json, fetch_json_post, normalize_transcript


class SearchService:
    def __init__(self, config: AppConfig, ollama_client: OllamaClient, location_service: LocationService):
        self.config = config
        self.ollama_client = ollama_client
        self.location_service = location_service

    def is_restaurant_request(self, user_text: str, keywords: tuple[str, ...]) -> bool:
        lowered = normalize_transcript(user_text)
        return any(contains_trigger_phrase(lowered, keyword) for keyword in keywords)

    def fetch_web_context(self, query_text: str, max_notes: int = 5) -> str:
        ...
        try:
            data = fetch_json(f"{self.config.web_search_url}?{params}", timeout=15)
    ...
    def ask_with_web_notes(
        ...
        web_context = self.fetch_web_context(query_text)
        answer = self.ollama_client.ask_with_messages(
            messages=[
    ...
    def build_restaurant_query(self, user_text: str, location: dict[str, Any] | None) -> str:
    ...
    def build_restaurant_prompt(self, user_text: str, web_context: str, location: dict[str, Any] | None) -> str:
        location_label = (
            str(location.get("area_label") or location.get("label")) if location else "the user's current area"
        )
        coordinates_line = ""
    ...
    def fetch_nearby_restaurants(self, location: dict[str, Any], limit: int = 5) -> list[dict[str, str]]:
        latitude = location.get("latitude")
        longitude = location.get("longitude")
        if latitude is None or longitude is None:
            raise RuntimeError("Location did not include usable coordinates for nearby restaurant search.")
...
out center tags;
"""
        endpoints = [self.config.overpass_url, self.config.overpass_fallback_url]
        elements: list[Any] = []
    ...
    def build_restaurant_answer(self, restaurants: list[dict[str, str]], location: dict[str, Any]) -> str:
        ...
        lines.append("These suggestions are based on location and nearby online map data, so they are approximate.")
        return "\n".join(lines)
    ...
    def fetch_restaurants_from_web_search(
        ...
        search_query = self.build_restaurant_query(user_text, location)
        web_context = self.fetch_web_context(search_query, max_notes=8)
        return self.ollama_client.ask_with_messages(
            messages=[
                {"role": "system", "content": WEB_SYSTEM_PROMPT},
                *(history_messages or []),
                {"role": "user", "content": self.build_restaurant_prompt(user_text, web_context, location)},
            ],
        )

    def serialize_restaurant_result(self, restaurants: list[dict[str, str]]) -> str:
